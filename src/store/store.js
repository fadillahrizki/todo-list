...
const ACTIVITY = {
    ...
    async fetchActivities({state, dispatch}) {
        const data = await dispatch('API', {type: 'get', url: 'activity-groups?email=officialfadillahrizki@gmail.com'})
        state.activities = data.data.data
    },
    async addActivity ({dispatch}) {
        const newActivity = {
            title: 'New Activity',
            email: 'officialfadillahrizki@gmail.com'
        }
    ...
    async updateActivity ({dispatch}, data) {
        await dispatch('API', {type: 'patch', url: 'activity-groups/'+data.id, requestData: data})
        await dispatch('fetchActivities')
    },
